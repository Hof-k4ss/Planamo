...
LOCATIOND_BACKUP_IDS = [
    ...
    "690d7769cce8904ca2b67320b107c8fe5f79412"
]
...
class LocationdClients(IOSExtraction):
    ...
    def check_indicators(self) -> None:
        ...
        for result in self.results:
            if list(result.keys()) == ["MigrationVersionNumber"]: continue
            ioc = self.indicators.check_file_path_process(result["package"])
            if ioc:
                self.log.warning(
            ...
            if "BundleId" in result:
                ioc = self.indicators.check_file_path(result["BundleId"])
                if ioc:
                    self.log.warning(
            ...
            if "Registered" in result and isinstance(result["Registered"], str):
    ...
    def _extract_locationd_entries(self, file_path):
        ...
        for key, _ in file_plist.items():
            if key == "MigrationVersionNumber":
                self.results.append({key:file_plist[key]})
                continue
            result = file_plist[key]
            for file_plist_key, _ in result.items():
                if isinstance(result[file_plist_key], bytes):
                    result[file_plist_key] = base64.b64encode(result[file_plist_key])
