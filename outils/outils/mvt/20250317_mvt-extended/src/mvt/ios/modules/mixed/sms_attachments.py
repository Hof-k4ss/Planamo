...
class SMSAttachments(IOSExtraction):
        ...
        if record["missing"]:
            if record["type"] == "start":
                return {
                    ...
                    "event": "sms_attachment_missing_start",
                    "data": f"Begining of a time frame with {record['amount']} missing attachment{'s' if record['amount'] > 1 else ''}",
                }
            else:
                return {
                    "timestamp": record["isodate"],
                    "module": self.__class__.__name__,
                    "event": "sms_attachment_missing_end",
                    "data": f"End of a time frame with {record['amount']} missing attachment{'s' if record['amount'] > 1 else ''}",
                }
    ...
    def check_indicators(self) -> None:
        for attachment in self.results:
            if attachment.get("missing", False): # support for legacy outputs
                continue
    ...
    def run(self) -> None:
        ...
        for item in cur:
            ...
            if (attachment["uti"] == "com.apple.watchface"):
                self.log.warning("Suspicious iMessage attachment (watchface - check Triangulation) %s on %s",
                                 attachment['filename'], attachment['isodate'])
                self.detected.append(attachment)

            missingAttachmentsAmount = attachment["attachment_id"] - previous_id - 1
            if missingAttachmentsAmount > 0:
                if missingAttachmentsAmount >= 5:
                    self.log.warning(
                        "%s attachment%s deleted between %s and %s",
                        missingAttachmentsAmount,
                        "s" if missingAttachmentsAmount > 1 else "",
                        previous_date,
                        attachment["isodate"],
                    )
                ...
                missingAttachment={
                    ...
                    "amount" : missingAttachmentsAmount,
                    "type" : "start"
                }
                self.results.append(missingAttachment)

                missingAttachment={
                    "missing" : True,
                    "isodate" : attachment["isodate"],
                    "amount" : missingAttachmentsAmount,
                    "type" : "end"
                }
                self.results.append(missingAttachment)

            previous_id = attachment["attachment_id"]
            previous_date = attachment["isodate"]
