...
class NetBase(IOSExtraction):
    ...
    def serialize(self, record: dict) -> Union[dict, list]:
        ...
        if record["proc_name"] is None:
            self.log.warning("Empty proc_name for bundle_name %s at %s", record['bundle_id'], record["live_isodate"])
            record["proc_name"] = ""
    ...
    def _find_suspicious_processes(self):
        self._find_suspicious_processes_fs()
        for proc in self.results:
            if proc.get("proc_name") == "com.apple.WebKit.WebContent":
                self.log.warning("\"com.apple.WebKit.WebContent\" doing network communications at %s. If this date is recent, you should be worried !", proc['live_isodate'])
                self.detected.append(proc)
            if proc.get("proc_name") == "MessagesBlastDoorService":
                self.log.warning("\"MessagesBlastDoorService\" doing network communications at %s. You should be worried !", proc['live_isodate'])
                self.detected.append(proc)
            if not proc["proc_name"]:
                ...
                suspicious_process_pattern_1 = re.compile('^/[0-9A-F\-]{16}$')
                suspicious_process_pattern_matched = suspicious_process_pattern_1.search(proc.get("proc_name", ""))
                if suspicious_process_pattern_matched:
                    self.log.warning("\"%s\" doing network communications at %s.", proc["proc_name"], proc['live_isodate'])
                    self.detected.append(proc)
            if not proc["bundle_id"] and not proc["live_proc_id"]:
                self.log.info(
    ...
    def _find_suspicious_processes_fs(self):
            ...
            if not proc["bundle_id"]:
                ...
                for file in files:
                    if not self.is_archive:
                        ...
                        if not file.is_dir() and proc.get("proc_name") and os.path.basename(file.filename) == proc["proc_name"]:
                            binary_path = file.filename
                            break


                if binary_path:
                    self.log.debug("Located at %s", binary_path)
                elif proc.get("proc_name"):
                    msg = (
