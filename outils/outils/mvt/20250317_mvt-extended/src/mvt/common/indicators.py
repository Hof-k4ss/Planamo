...
class Indicators:
    ...
    def _process_indicator(self, indicator: dict, collection: dict) -> None:
        key = indicator.get("pattern", "").strip("[]").split("=")[0]
        value = "=".join(indicator.get("pattern", "").strip("[]").split("=")[1:])
    ...
    def check_url(self, url: str) -> Union[dict, None]:
        ...
        for ioc in self.get_iocs("urls"):
            if (re.search(ioc["value"], trim_prefix(url,"/"))):
                self.log.warning("Found a known suspicious URL (by regex) \"%s\" matching indicators from \"%s\"",
                                 url, ioc["name"])
                return ioc
    ...
    def check_file_path(self, file_path: str) -> Union[dict, None]:
        ...
        ioc = self.check_file_name(file_path)
        if ioc:
            return ioc
        ...
        for ioc in self.get_iocs("file_paths"):
            ...
            if (re.match(ioc["value"], trim_prefix(file_path,"/"))):
                self.log.warning("Found a known suspicious file path (by regex) \"%s\" matching indicators from \"%s\"",
                                 file_path, ioc["name"])
                return ioc
