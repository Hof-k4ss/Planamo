...
def convert_epoch_to_unix(timestamp):
    """Convert Epoch microseconds timestamp to datetime"""
    try:
        return datetime.datetime(1970, 1, 1) + datetime.timedelta(seconds=timestamp/1000000.)
    except OverflowError:
        pass
    return ""

def convert_mobilecontainermanagerlog_to_unix(timestamp_string):
    locale.setlocale(locale.LC_ALL, "C")
    return(datetime.datetime.strptime(timestamp_string, "%a %b %d %H:%M:%S %Y"))

def convert_launchdlog_to_unix(timestamp_string):
    locale.setlocale(locale.LC_ALL, "C")
    return(datetime.datetime.strptime(timestamp_string, "%Y-%m-%d %H:%M:%S.%f"))

def convert_lockdowndlog_to_unix(timestamp_string):
    locale.setlocale(locale.LC_ALL, "C")
    return(datetime.datetime.strptime(timestamp_string, "%m/%d/%y %H:%M:%S.%f"))
...
def recursive_resolve(root):
    if isinstance(root, ccl_bplist.NsKeyedArchiverDictionary):
        keys = list(root.keys()).copy()

        if sorted(keys) == ["root"]:
            return(recursive_resolve(root["root"]))

        if sorted(keys) == ["$class", "NS.objects"]:
            return(recursive_resolve(root["NS.objects"]))

        if sorted(keys) == ["$class", "NS.data"]:
            return(recursive_resolve(root["NS.data"]))

        if sorted(keys) == ["$class", "NS.keys", "NS.objects"]:
            entry = {}
            for i in range(len(root["NS.keys"])):
                key = str(recursive_resolve(root["NS.keys"][i]))
                value = recursive_resolve(root["NS.objects"][i])
                entry[key] = value
            return(entry)

        for key in keys:
            value = root[key]
            if key in ["$class", "$classes", "backingStore", "relationship", "home", "HM.home", "HM.user", "accessory", "service", "metadata", "_codableEnum", "_codableDescription"]:
                root.pop(key)
            elif key == "NS.time":
                return(convert_mactime_to_iso(value))
            else:
                root[key] = recursive_resolve(value)
        return root

    if isinstance(root, ccl_bplist.NsKeyedArchiverList):
        entry = []
        for item in root:
            entry.append(recursive_resolve(item))
        return entry

    if isinstance(root, int) or isinstance(root, float) or isinstance(root, str):
        if root == "$null": return ""
        return root
    ...
    if isinstance(root, bytes):
        ...
        with open(temp_file_path, "wb") as f:
            f.write(root)
        with open(temp_file_path, "rb") as temp_handle:
            try:
                temp_plist = ccl_bplist.load(temp_handle)
            except ccl_bplist.BplistError as e:
                if str(e) == "Bad file header":
                    try:
        ...
        temp_dir.cleanup()
        try:
            temp_ns_keyed_archiver_obj = ccl_bplist.deserialise_NsKeyedArchiver(temp_plist, parse_whole_structure=True)
        except ValueError as e:
            return(recursive_resolve(temp_plist))
        except TypeError as e:
            return(recursive_resolve(temp_plist))
        return(recursive_resolve(temp_ns_keyed_archiver_obj))
    ...
    if isinstance(root, datetime.datetime):
        return convert_datetime_to_iso(root)
