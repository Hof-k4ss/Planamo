...
class SMS(IOSExtraction):
    ...
    def serialize(self, record: dict) -> Union[dict, list]:
        ...
        # If the message was delivered, we add an extra event.
        if record.get("isodate_delivered"):
            records.append(
            {
                "timestamp": record["isodate_delivered"],
                "module": self.__class__.__name__,
                "event": "sms_delivered",
                "data": sms_data,
            })
    ...
    def run(self) -> None:
        ...
        for item in items:
            ...
            if message.get("date_delivered", 0) != 0: message["isodate_delivered"] = convert_mactime_to_iso(message["date_delivered"])
            if message.get("date_read", 0) != 0: message["isodate_read"] = convert_mactime_to_iso(message["date_read"])
            message["direction"] = (
