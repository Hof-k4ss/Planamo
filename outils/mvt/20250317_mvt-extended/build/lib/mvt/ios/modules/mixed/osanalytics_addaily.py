...
class OSAnalyticsADDaily(IOSExtraction):
    ...
    def _find_suspicious(self) -> None:
        for result in self.results:
            ...
            if result.get("package") == "com.apple.WebKit.WebContent":
                self.log.warning("\"com.apple.WebKit.WebContent\" at %s. If this date is recent, you should be worried", result['ts'])
                self.detected.append(result)
            if result.get("package") == "MessagesBlastDoorService":
                self.log.warning("\"MessagesBlastDoorService\" doing network communications at %s. You should be worried !", result['ts'])
                self.detected.append(result)
            suspicious_process_pattern_1 = re.compile('^/[0-9A-F\-]{16}$')
            suspicious_process_pattern_matched = suspicious_process_pattern_1.search(result.get("package", ""))
            if suspicious_process_pattern_matched:
                self.log.warning("\"%s\" doing network communications at %s.", result.get("package"), result.get("ts"))
                self.detected.append(result)
