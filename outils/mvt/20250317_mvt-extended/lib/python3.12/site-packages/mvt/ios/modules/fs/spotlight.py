...
from mvt.common.utils import convert_epoch_to_unix, convert_datetime_to_iso

from ..base import IOSExtraction

SPOTLIGHT_FOLDER_PATH = [
    "private/var/mobile/Library/Spotlight/CoreSpotlight/*/index.spotlightV2/store.db",
]

SPOTLIGHT_ARCHIVE_FOLDER_PATH = [
    "private/var/mobile/Library/Spotlight/CoreSpotlight/*/index.spotlightV2/dbStr-*.map.data",
    "private/var/mobile/Library/Spotlight/CoreSpotlight/*/index.spotlightV2/dbStr-*.map.header",
    "private/var/mobile/Library/Spotlight/CoreSpotlight/*/index.spotlightV2/dbStr-*.map.offsets"
]
...
class FileMetaDataListing:
    ...
    def ReadDate(self):
        ...
        try:
            return convert_datetime_to_iso(datetime.datetime(2001,1,1) + datetime.timedelta(seconds = mac_abs_time))
...
class SpotlightStore:
    ...
    def ParseMetadataBlocks(self, items, items_to_compare=None, process_items_func=None):
...
def ProcessStoreDb(input_file_path):
    ...
    try:
        ...
        return(store.ParseMetadataBlocks(items, None, None))
...
class SpotlightFolder(IOSExtraction):
    ...
    def check_indicators(self) -> None:
        ...
        for result in self.results:
            for key in list(result.keys()).copy():
                ioc = self.indicators.check_process(str(result[key]))
                if ioc:
                    result["matched_indicator"] = ioc
                    if result not in self.detected:
                        self.log.warning("Found mention of a malicious process \"%s\" : %s at %s",
                                            ioc["value"], result[key], result["Last_Updated"])
                    ...
                    if ioc["value"] in str(result[key]):
                        ...
                        if result not in self.detected:
                            self.log.warning("Found mention of a malicious domain \"%s\" : %s at %s",
                                             ioc["value"], result[key], result["Last_Updated"])
                            self.detected.append(result)
                if ("tmp" in str(result[key]) or "bd_tool" in str(result[key])):
                    self.log.warning("Found mention of tmp* / bd_tool* : %s at %s", result[key], result["Last_Updated"])

    def _cast_items(self) -> None:
        for item in self.items:
            try:
                ...
                entry = {
                    ...
                    "Parent_Inode_Num": item.parent_id,
                    "Last_Updated" : convert_datetime_to_iso(convert_epoch_to_unix(item.date_updated)),
                    "data" : str(item.meta_data_dict)
                }
    ...
    def run(self) -> None:
        if self.is_archive: all(self._copy_fs_files_from_patterns(SPOTLIGHT_ARCHIVE_FOLDER_PATH))
        for file_path in self._get_fs_files_from_patterns(SPOTLIGHT_FOLDER_PATH):
            self.file_path = file_path
            self.log.info("Found Spotlight folder at path: %s", file_path)
            self.items = ProcessStoreDb(self.file_path)
            self._cast_items()

        if self.results: self.results = sorted(self.results, key=lambda entry: entry["Last_Updated"])
        self.log.info("Extracted %d records from Spotlight", len(self.results))
