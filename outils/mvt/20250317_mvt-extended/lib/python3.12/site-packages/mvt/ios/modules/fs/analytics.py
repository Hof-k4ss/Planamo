...
class Analytics(IOSExtraction):
    ...
    def find_suspicious(self) -> None:
        for record in self.results:
            if record.get("service", "") == "com.apple.private.alloy.fmf":
                self.log.warning("Event of type \"%s\" by service \"%s\" on %s", record.get("eventType", ""), record.get("service", ""), record.get("isodate", ""))
    ...
    def check_indicators(self) -> None:
        ...
        for result in self.results:
            for value in result.values():
                ...
                if ("tmp" in value or "bd_tool" in value):
                    self.log.warning("Found mention of tmp* / bd_tool* : \"%s\" in %s at %s", value, result["artifact"], result["timestamp"])
