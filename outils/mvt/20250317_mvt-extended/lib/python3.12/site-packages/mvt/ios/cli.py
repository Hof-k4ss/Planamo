...
@click.option("--output", "-o", type=click.Path(exists=False), help=HELP_MSG_OUTPUT)
@click.option("--fast", "-f", is_flag=True, help=HELP_MSG_FAST)
@click.option("--fsevents", "-F", is_flag=True, help=HELP_MSG_FSEVENTS)
@click.option("--list-modules", "-l", is_flag=True, help=HELP_MSG_LIST_MODULES)
@click.option("--module", "-m", help=HELP_MSG_MODULE)
@click.option("--hashes", "-H", is_flag=True, help=HELP_MSG_HASHES)
@click.option("--verbose", "-v", is_flag=True, help=HELP_MSG_VERBOSE)
...
@click.pass_context
def check_fs(ctx, iocs, output, fast, fsevents, list_modules, module, hashes, verbose, dump_path):
    set_verbose_logging(verbose)
    module_options = {"fast_mode": fast, "fsevents" : fsevents}
...
#==============================================================================
@cli.command("check-sysdiagnose", help="Extract artifacts from a sysdiagnose")
...
@click.option("--hashes", "-H", is_flag=True, help=HELP_MSG_HASHES)
@click.option("--verbose", "-v", is_flag=True, help=HELP_MSG_VERBOSE)
@click.argument("SYSDIAGNOSE_PATH", type=click.Path(exists=False, file_okay=True, dir_okay=False))
@click.pass_context
def check_fs(ctx, iocs, output, fast, list_modules, module, hashes, verbose, sysdiagnose_path):
    set_verbose_logging(verbose)
    module_options = {"fast_mode": fast}
    cmd = CmdIOSCheckSysdiagnose(target_path=sysdiagnose_path, results_path=output,
                        ioc_files=iocs, module_name=module, module_options=module_options, hashes=hashes)

    if list_modules:
        cmd.list_modules()
        return

    log.info("Checking sysdiagnose located at: %s", sysdiagnose_path)

    cmd.run()

    if len(cmd.timeline_detected) > 0:
        log.warning("The analysis of the sysdiagnose produced %d detections!",
