...
class WebkitResourceLoadStatistics(IOSExtraction):
    ...
    def check_indicators(self) -> None:
        ...
        for result in self.results:
            if isinstance(result, str):
                ...
                for result_item in self.results[result]:
                    ioc = self.indicators.check_url(result_item["registrable_domain"])
                    if ioc:
    ...
    def run(self) -> None:
        ...
        self.log.info("Extracted a total of %d WebKit resource load statistics records", len(self.results))